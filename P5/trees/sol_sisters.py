...
def sisters(nodes):
    ...
    for i in nodes:
        i_left = nodes[i].left
        i_right = nodes[i].right
        if i_left and i_right and not nodes[i_left].left and not nodes[i_right].left:
            output.append((i_left, i_right, i))  # i_left, i_right: sisters, i: parent
    return output
